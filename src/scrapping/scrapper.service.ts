...
export class ScrapperService {
    ...
    constructor({elpaisService, elmundoService}: { elpaisService: NewsSite; elmundoService: NewsSite }) {
    ...
    async getAndSaveAllFeeds(limit?: number): Promise<IFeed[]> {
        try {
            ...
            const elpaisFeeds: IFeed[] = (limit ? newsElpais.slice(0, limit) : newsElpais).map(mapArticleToFeed)
            const elmundoFeeds: IFeed[] = (limit ? newsElmundo.slice(0, limit) : newsElmundo).map(mapArticleToFeed)
