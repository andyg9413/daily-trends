    ...
    private async saveArticlesAsFeeds(articles: IArticle[]): Promise<void> {
        try {
            for (const article of articles) {
                const existingFeed = await this.feedService.findOneFeed({description: article.body, isDeleted: false});
                if (existingFeed) {
                    continue;
                }
                await this.feedService.addFeed({ ...article, datePublished: new Date(Date.now()), scrapped: true, description: article.body, title: article.title || "" });
            }
        } catch (error) {
